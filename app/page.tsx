...
export default function HomePage() {
	...
	const [welcomeTitle, setWelcomeTitle] = useState("수능국어는 이정관");
	...
	useEffect(() => {
		...
		const fetchHomeData = async () => {
			...
			if (!settingResult.error && settingResult.data) {
				...
				setWelcomeTitle(setting.welcome_title?.trim() || "수능국어는 이정관");
	...
	const vocabPair: (typeof DAILY_VOCAB_LIST)[number][] =
		n <= 1 ? [DAILY_VOCAB_LIST[0]] : [DAILY_VOCAB_LIST[offset], DAILY_VOCAB_LIST[(offset + 1) % n]];
	...
	return (
		<main className="relative min-h-screen min-h-[100dvh] bg-app-sage pb-[calc(5.75rem+env(safe-area-inset-bottom))]">
			<div className="border-b border-slate-200/80 bg-white pt-[max(0.5rem,env(safe-area-inset-top))] pb-3">
				<div className={`${STUDENT_APP_SHELL}`}>
					<div
						...
						aria-label="브랜드 안내"
					>
						<p className="text-base font-bold tracking-tight text-white sm:text-lg">{welcomeTitle}</p>
					</div>
				</div>
			</div>
			...
			<div className={`${STUDENT_APP_SHELL} space-y-5 pt-5`}>
				<div className={`${studentComicCard} p-2 sm:p-2.5`} onMouseLeave={() => setActiveVocabWord(null)}>
				...
				<section>
					<div className="mb-2 flex items-center justify-between">
						<h2 className="text-sm font-bold text-slate-800">최신 학습 자료</h2>
						<Link href="/material" className="text-xs font-semibold text-slate-600 hover:text-slate-900">
							전체보기
						</Link>
					</div>
					{isLoading ? <p className="text-sm text-slate-500">불러오는 중…</p> : null}
					{!isLoading && !errorMessage && latestMaterials.length === 0 ? (
						<p className="text-sm text-slate-500">등록된 최신 자료가 없습니다.</p>
					) : null}
					{!isLoading && !errorMessage && latestMaterials.length > 0 ? (
						<ul className="space-y-2">
							{latestMaterials.map((material) => (
								<li key={material.id}>
									...
									>
										...
										<div className="min-w-0 flex-1">
											<p className="line-clamp-2 text-xs font-semibold leading-snug text-slate-900">{material.title}</p>
											<div className="mt-0.5 flex flex-wrap items-center gap-2 text-[10px] text-slate-500">
												<span className="font-medium text-brand/90">{material.category}</span>
